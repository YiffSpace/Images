...
export async function scheduleGravatarPurge(hash: string, delayMs = GRAVATAR_PURGE_DELAY_MS): Promise<void> {
    ...
    Log("avatars:gravatar:purge", `Scheduled purge for ${hash} at ${new Date(runAt).toISOString()}`);
}
        ...
        await withContext(async () => {
            ...
            do {
                ...
                for (const hash of hashes) {
                    ...
                    await remove(hash);
                    Log("avatars:gravatar:purge", `Purged ${hash}`);
                }
            } while (hashes.length === GRAVATAR_PURGE_BATCH_SIZE);
        });
    } catch (error) {
        Log("avatars:gravatar:purge", "Purge worker failed: %O", error);
    } finally {
        running = false;
    }
}
